    ...
    def get(self, request):
        ...
        if template_id:
            ...
            if template_qs.exists():
                ...
                if "delivered" in str(data['deliverystatus']).lower() or "pickedup" in str(data['deliverystatus']).lower() or "confirmed" in  str(data['deliverystatus']).lower() or  "attempted" in str(data['deliverystatus']).lower() or "statusCodeAdded" in str(data['deliverystatus']).lower() and (data['start_date'] and data['end_date']):
                    orders = orders.filter(
                        Q(delivery__gte=data['start_date'], delivery__lte=data['end_date']) | 
                        Q(departed__gte=data['start_date'], departed__lte=data['end_date'])
                    )
    ...
    def post(self, request): 
        ...
        if not filter_form.is_valid() or not report_form.is_valid():
            try:
                ...
                generated = GeneratedReport(user = request.user)
                generated.deliverystatus = statuss
                generated.start_date = start_date
                generated.report_type = fileFormat
                generated.end_date = end_date
                generated.customerstate = customerstate
                generated.customercity = customercity
                generated.address = address
                generated.customername = customername
                generated.save()

                report = ReportFilter()
                report.report_filter = generated
                report.save()
                ...
                report_filter.start_date = start_date
                report_filter.end_date = end_date
                report_filter.customerstate = customerstate
                report_filter.customercity = customercity
                report_filter.address = address
                report_filter.customername = customername


                report_filter.report = generated
                report_filter.save()
...
def download_report(request, report_id):
    ...
    if report:
        ...
        try:
            delivery= report.deliverystatus.all() if report.deliverystatus else []
            delivery_status = str(delivery).lower().strip('"[]"').replace("'", "").split(", ")
        except:
            delivery_status = str(report.deliverystatus).lower().split(", ")
        data = {
            'delivery_status': delivery_status,
            'start_date': report_filter.start_date,
            'end_date': report_filter.end_date,
            'customer_city': report_filter.customercity,
            'customer_state': report_filter.customerstate
        }
        ...
        for field_name, value in data.items():
            if value:
                if field_name == 'delivery_status':
                    print(value)
                    orders = orders.filter(deliverystatus__in = value)
                    start_date = datetime.strptime(str(data['start_date']), "%Y-%m-%d %H:%M:%S%z").date() if data['start_date'] else None
                    end_date = datetime.strptime(str(data['start_date']), "%Y-%m-%d %H:%M:%S%z").date() if data['start_date'] else None
                    if value in ["delivered", "statuscodeAdded", "attempted", "pickedup", "confirmed", "placed"] and (start_date and end_date):
                        orders = orders.filter(
                            Q(delivery__gte=start_date, delivery__lte=end_date) | 
                            Q(departed__gte=start_date, departed__lte=end_date))
                    elif start_date and not end_date:
                        ...
                        orders = orders.filter(
                            Q(delivery__range=(start_date, end_date)) | 
                            Q(departed__range=(start_date, end_date)))
                if field_name == 'customer_city':
                    print(value)
                    orders = orders.filter(customercity__icontains=value)
                if field_name == 'customer_state':
                    orders = orders.filter(customerstate__icontains=value)
                if field_name == '':
                    orders = orders.filter(report__icontains={field_name: value})
...
def report_history(request):
    ...
    filters = {
        ...
        'start_date': request.GET.get('start_date'),
        'end_date': request.GET.get('end_date'),
        'customer_state': request.GET.get('customerstate'),
        'customer_city': request.GET.get('customercity'),
    }
    ...
    for field_name, value in filters.items():
        if value:
            if field_name == 'delivery_status':
                q_objects &= Q(deliverystatus=value)
            elif field_name in ['start_date', 'end_date']:
                q_objects &= _handle_date_filter(field_name, value, q_objects)
            else:
                q_objects &= Q(report_filter__icontains={field_name: value})
...
def _handle_date_filter(field_name, value, q_objects):
    if field_name == 'start_date':
        return q_objects | Q(report_filter__start_date__gte=value)
    elif field_name == 'end_date':
        return q_objects & Q(report_filter__end_date__lte=value)
    else:
        raise ValueError(f"Invalid date filter field: {field_name}")

# an api that gets template information
def add_report_template(request):
    template_form = TemplatedFiltersForm(request.POST)
    if not template_form.is_valid():
        data = {
            'deliverystatus': request.POST.get("deliverystatus").split(","),
            'start_date': request.POST.get('start_date'),
            'end_date': request.POST.get('end_date'),
            'customerstate': request.POST.get('customerstate'),
            'customercity': request.POST.get('customercity'),
        ...
        template = TemplatedFilters(user = request.user)
        template.deliverystatus = data['deliverystatus']
        template.start_date = data['start_date']
        template.end_date = data['end_date']
        template.customerstate = data['customerstate']
        template.customercity = data['customercity']
    ...
    if template_form.is_valid():
        template_instance = template_form.save(commit=False)
        template_instance.user = request.user
        template_instance.save()
        # return json django json response
        return JsonResponse({'message': 'Template added successfully'}, status=201)
    return JsonResponse({'message': template_form.errors}, status=400)
...
def download_excel_report(request, queryset):
    ...
    for row in queryset:
        try:
            ...
            values = []
            for delivery_status in delivery_statuses:
            # Prepare values for each delivery status
                current_values = [
                    row.id,
                    row.externalid,
                    delivery_status.strip(),
                    ...
                    # Convert datetimes to timezone-naive format (if necessary)
                for value in __iter__(current_values):
                    if isinstance(value, datetime):
                        current_values = str(current_values)  # Convert to string

                values.append(current_values)  # Append values for this delivery status
            data.append(values) 
        except:
            values = []
            for field in header_names:
...
class CarrierFollowUp(UpdateView):
    ...
    success_url = reverse_lazy('carriers')

    def get_initial(self, **kwargs):
        carrier = self.model.objects.get(id = self.kwargs['pk'])
        
        # This method sets the initial choice selections for each field
        initial = super().get_initial()
        carrier = self.get_object()

        followup = self.request.session.get(f'fllowup_{carrier.pk}')
        if followup:
            initial.update(followup)
        else:
            initial['usservice'] = ast.literal_eval(carrier.usservice) if carrier.usservice else []
            initial['canservice'] = ast.literal_eval(carrier.canservice) if carrier.canservice else []
            initial['mexservice'] = ast.literal_eval(carrier.mexservice) if carrier.mexservice else []
    ...
    def form_valid(self, form):
        ...
        if f'fllowup_{carrier.pk}' in self.request.session:
            del self.request.session[f'fllowup_{carrier.pk}']

        return response
    
    def form_invalid(self, form):
        messages.error(self.request, 'There was an error updating the carrier.')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if 'save' in request.POST:
                self.request.session[f'fllowup_{self.object.pk}'] = form.cleaned_data

                return HttpResponseRedirect(reverse('carriers') + '?success=followup')
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get('success') == 'followup':
            # Add a success message to the context data
            messages.add_message(self.request, messages.SUCCESS, 'Use same link to follow up later. If you need assistance please contact info@357company.com.')
...
class CarrierCreate(CreateView):
    ...
    def get_queryset(self):
        email = self.request.POST.get('email', None)  # Get email from the form data
        if email:
            return Carrier.objects.filter(email=email)  # Filter by email if present
        else:
    ...
    def post(self, request, *args, **kwargs):
        carrier = None  # Get the current Carrier object

        email = self.request.POST.get('email', None)  # Get email from the form data\
        print(email)
        if email:
            try:
                carrier = Carrier.objects.get(email=email)
                print(carrier.pk)
                from django.http import HttpResponseRedirect

                if 'save' in request.POST:
                    mutatble_post = reqeust.POST.copy()
                    self.request.session['fllowup'] = mutatble_post
                    # send_mail(
                    #     'Your Carrier Onboarding Form has been Saved.',
                    #     # 'Quote Accepted for ' + str(carrier.cost),
                    #     'info@357company.com',
                    #     [f'{email}',],  # Using the email of the carrier
                    #     f'Your form data is saved and you can continue the form at anytime with this link: https://portal.357company.com/carrier/{carrier.pk}/followup/'
                    # )
                    return HttpResponseRedirect(reverse('carriers') + '?success=false')

                else:
                    return HttpResponseRedirect(reverse('carriers') + '?success=true')
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get('success') == 'true':
            # Add a success message to the context data
            messages.add_message(self.request, messages.SUCCESS, 'Carrier was created successfully. You will get an email from 357 Team member')

        if self.request.GET.get('success') == 'false':
            # Add a success message to the context data
            messages.add_message(self.request, messages.SUCCESS, 'Your form has been saved, a 357 Team member will follow up with you to complete the form shortly.')
